...
-- GIN (pg_trgm) on flattened alias text (use array_to_string for TEXT[] → text)
CREATE INDEX "FootballPlayer_aliases_gin_trgm_idx" ON "FootballPlayer" USING GIN ((array_to_string("aliases", ' ')) gin_trgm_ops);
