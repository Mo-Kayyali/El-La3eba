...
import { levenshtein } from './src/game/guess-matcher.util';

const prisma = new PrismaClient();

// Proposed matchToken implementation
function matchTokenProposed(gToken: string, tToken: string) {
  // Exact match
  ...
  if (tToken.startsWith(gToken) || gToken.startsWith(tToken)) {
    ...
    if (minLength >= 3) {
      // Prefix penalty is 0.5 plus penalty proportional to length discrepancy
...
// Proposed evaluateMatch implementation
function evaluateMatchProposed(guess: string, target: string) {
  ...
  const unmatchedTTokensPenalty = unmatchedTTokensCount * 0.05; // 0.05 penalty per unmatched target token
...
// Proposed candidate resolution & ambiguity check
async function resolveGuessProposed(guessName: string) {
  const normalizedGuess = guessName.trim().replace(/-/g, ' ');
  const guessLen = normalizedGuess.length;
  if (guessLen < 3) return { validCandidates: [], isAmbiguous: false, ambiguityReason: 'Length < 3' };
  ...
  // Sort by matchConfidence DESC, then SQL w_sim DESC as secondary tie-breaker
  ...
  // Ambiguity condition:
  // Ambiguous ONLY IF:
  // 1. Both top candidate and 2nd candidate exist
  // 2. Both candidates have EXACT token match quality (bestReason === 'exact')
  // 3. The confidence gap between them is strictly <= 0.01 (essentially identical match quality)
...
async function main() {
  ...
  const testCases = [
    ...
    // 4 Section 4 Sanity Check Cases
