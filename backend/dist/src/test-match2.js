...
function matchToken(gToken, tToken) {
    ...
    const maxTypos = Math.max(1, Math.floor(Math.max(gToken.length, tToken.length) / 4));
    if (dist <= maxTypos) {
...
function evaluateMatch(guess, target) {
    ...
    const tChunks = getTokenCombinations(tTokens);
    const gChunks = getTokenCombinations(gTokens);
    let score = 0;
    let matchedG = new Set();
    let matchedT = new Set();
    let matches = [];
    ...
    let matchedChars = finalMatches.reduce((sum, m) => sum + m.tStr.length, 0);
    ...
    let unmatchedTChars = totalTChars - matchedChars;
    let finalScore = totalPenalty + unmatchedGChars * 0.5 + unmatchedTChars * 0.5;
    console.log({ guess: g, target: t, matches: finalMatches, matchedChars, totalTChars, totalPenalty, finalScore });
    return finalScore;
}
const tests = [
...
for (const t of tests) {
    evaluateMatch(t.guess, t.stored);
}
//# sourceMappingURL=test-match2.js.map