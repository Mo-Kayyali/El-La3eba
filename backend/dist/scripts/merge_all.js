...
async function main() {
    ...
    const mergePath = path.join(dataDir, 'duplicate_players_audit.json');
    ...
    for (const pair of pairs) {
        ...
        if (!p1Exists || !p2Exists) {
            console.log(`Skipping pair ${pair.player1.name} / ${pair.player2.name} - one or both already merged.`);
            continue;
        }
        ...
        console.log(`Merged player ${pair.player1.name} & ${pair.player2.name} (kept ${primaryId === p1Id ? 'player1' : 'player2'})`);
        mergedCount++;
    }
...
//# sourceMappingURL=merge_all.js.map