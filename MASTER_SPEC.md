...
- **Admin CRUD:** Basic CRUD endpoints for `Competition` and `Club` under `/admin/competitions` and `/admin/clubs`, protected by `RolesGuard`. Create/Update endpoints strictly validate foreign keys (`countryCode`, `currentCompetitionId`), returning clean HTTP 400 Bad Request instead of raw DB errors. Delete endpoints catch Prisma `P2003` constraint violations, returning HTTP 409 Conflict if the entity is still referenced. Basic admin UI panels exist at `/admin/competitions` and `/admin/clubs`.
