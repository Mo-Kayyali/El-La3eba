...
- **Matchmaking timeouts:** Queue searches and private rooms are **strictly 60s**. Queue timeout emits `searchExpired` only for users actually purged by timestamp cutoff (no per-user `setTimeout` race). Private rooms (`private_room:{code}` + `user_room:{userId}`) use EX 60; host expiry cleanup emits `roomExpired`.
