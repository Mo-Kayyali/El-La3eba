...
- **Known Limitation (Best-of-N):** The majority-win threshold in the shared `checkBestOfNMatchWin` evaluator is currently hardcoded to 2, which is only correct for the fixed 3-round Ranked/Unrated composition. It must be generalized to `Math.ceil(composition.length / 2)` when Private Room variable-length compositions ship.
