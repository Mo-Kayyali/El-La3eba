...
- **Fuzzy Search Guess Resolution & Ambiguity Rules:** Guess resolution in `GameGateway` uses `gameService.guessPlayer(guessName)` which ranks candidate players strictly by `matchConfidence` DESC. Resolution locks strictly to `matchedPlayers[0]` (the top candidate). The gateway does NOT iterate down lower-ranked candidates to find a fallback match that happens to validate against the active question. Both `alreadyGuessed` and `isCorrect` are evaluated against `matchedPlayers[0]` exclusively:
  ...
  - **N-Way Group Ambiguity Condition (fully dynamic, no hardcoded names):** After sorting, the system computes `nearTopExactGroup` = all candidates within `ε = 0.001` of the top confidence score **and** matched via `bestReason === 'exact'` (not prefix/typo/substring). This group is computed entirely from the live query results at match time — there are no hardcoded name lists or special-cased strings anywhere in the code:
    - **Group size = 1:** Clean single match → resolve to that candidate.
    - **Group size = 2:** Two-way tie → use prominence tie-break (`aliasesCount + clubsCount → isMainNameMatch → w_sim`). Legitimate for near-duplicate entries or two candidates where one is clearly dominant in seeded data.
    - **Group size ≥ 3:** Multiple genuinely distinct people share this name → `isAmbiguous = true`. Player must type a more specific guess. Examples from real seeded data: `"salah"` (9-way tie), `"ahmed"` / `"ali"` / `"omar"` (17–29-way ties). When ambiguous, `matchedPlayer` is set to `null` and registers an unmatched/ambiguous input strike.
  - **Admin Dashboard Question Test Guess Diagnostics (standing debugging aid):** The Admin Question Editor's POST `/admin/questions/:id/test-guess` endpoint uses the exact same `gameService.guessPlayer()` and strict `matchedPlayers[0]` validation logic as live matches. It returns detailed candidate diagnostics for the top 5 fuzzy matches: player name, matched target, confidence %, question validation status, and picked/ambiguous flags. The Admin UI renders a full diagnostics card — this tool is a standing debugging aid for any future fuzzy-search or guess-resolution investigation and should be kept.
