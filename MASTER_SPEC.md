...
- **Private Room Config Flow:** Private rooms support custom rule sets configured by the host via a UI modal before sending an invite. The configuration (`composition: GameMode[]`, `timerConfig: Record<string, number>`) is validated and stored in Redis by the matchmaking service (`createPrivateRoom`). When the invitee accepts (sending only a bare accept with `inviterId`), the backend resolves the room and dynamically initializes the `GameState` envelope (`mode`, `composition`, `timerConfig`) using the host's stored configuration. The frontend displays a summary of these rules in the non-blocking invite popup.
