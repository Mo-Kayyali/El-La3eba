...
- **Auth / session hydration:** Zustand `persist` uses `skipHydration: true` plus a `bootstrapped` flag in `frontend/lib/auth-store.ts`. `AuthSessionProvider` rehydrates from `localStorage`, syncs `axios` default `Authorization`, then calls `GET /auth/me` when a token exists before marking `bootstrapped` true — protected routes wait on this so a hard refresh does not flash “logged out”. Shared REST helpers live in `frontend/lib/api.ts` (Bearer interceptor + `refreshAuthProfile()`).
