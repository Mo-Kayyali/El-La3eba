...
import { Copy, Trophy, Swords, Percent } from "lucide-react";
...
export default function PublicProfilePage() {
  ...
  useEffect(() => {
    ...
    if (!accessToken || !isAuthenticated || !id) {
  ...
  }, [accessToken, bootstrapped, id, isAuthenticated, router]);
  ...
  if (!bootstrapped || !id) {
    return (
      <div className="min-h-screen bg-[#030712] text-slate-100 flex items-center justify-center">
        <span className="h-8 w-8 animate-spin rounded-full border-2 border-sky-400/40 border-t-sky-400" />
      </div>
    );
  }
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-400">
                A quick look at this player’s record.
              </p>
            </div>
          ...
          <div className="mt-8 rounded-2xl border border-white/10 bg-black/20 p-4 sm:p-5">
            ...
            <div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button
                ...
                onClick={handleCopy}
                disabled={!profile?.id}
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/10 px-3 py-2 text-sm font-semibold text-slate-100 transition hover:bg-white/15 disabled:cursor-